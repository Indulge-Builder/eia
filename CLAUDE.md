...
src/lib/elaya/python-brain.ts       ← runPythonBrainTurn()/isPythonBrainConfigured() — THE Node→Python-brain transport (bearer BRAIN_API_SECRET → ELAYA_BRAIN_URL/v1/elaya/chat, SSE pump via lib/elaya/sse.ts, typed rejections cap/duplicate/unauthorized/unavailable; https:// REQUIRED in production — the CloudFront front on the Fargate ALB). Passes ONLY a verified profile id. Never a second brain client
